classdef cj < handle
    ...
    % Date:     17 Nov 2016
    % About:
    % This class handles instances of cylindrical joint (cj) constraints. It Uses 
    % the r-p formulation (euler parameters). Removes 4 degrees of Freedom.
    %
    % cylindrical joint constraint: 
    % Geometrically requires that two vectors on two bodies should stay
    % collinear at all time. If body j is fixed, body i can slide and
    % rotate around a specified axis, and therefore has 2 degrees of
    % freedom of relative motion.
    
    properties
        system; % parent system3D object to which this constraint is a member. 
        rDOF = 4; % removes 5 degree of freedom
        bodyi;  % body i
        bodyj;  % body j
    ...
    methods
        ...
        function cons = cj(system,bodyi,PiID,aBari_tail,aBari_head,bBari_tail,bBari_head,...
            ...
            % from ME751_f2016 slide 29 from lecture 09/26/16
            cons.subCons{1} = constraint.p1(cons.system,cons.bodyi,cons.aBari_tail,cons.aBari_head,...
                cons.bBari_tail,cons.bBari_head,cons.bodyj,cons.cBarj_tail,cons.cBarj_head);
            cons.subCons{2} = constraint.p2(cons.system,cons.bodyi,cons.aBari_tail,cons.aBari_head,...
                cons.bBari_tail,cons.bBari_head,cons.Pi,cons.bodyj,cons.Qj);

            if abs(cons.phi) > 1e-4
                warning('Initial conditions for ''cj'' are not consistent. But solution will converge so constraints are satisfied.')
            end
        end        
        ...
        function phi_r = get.phi_r(cons) % partial derivative of constraint with respect to r
            % from ME751_f2016 slide 29 from lecture 09/26/16
            % phi_r : [4x6] normally, unless grounded, then [4x3]
        ...
        function phi_p = get.phi_p(cons) % partial derivative of constraint with respect to p
            % from ME751_f2016 slide 29 from lecture 09/26/16
            % phi_p : [4x8] normally, unless grounded, then [4x4]
        ...
        function phiLambda_rr = phiLambda_rr(cons,lambda)
            ...
            % phiLambda_rr : [24x6] normally, unless grounded, then [12x3]
            % inputs:
            %    lambda : [4x1] vector of lambda values, corresponding to
            %    the subconstraints
            
            phiLambda_rr = [cons.subCons{1}.phiLambda_rr(lambda(1:2)); cons.subCons{2}.phiLambda_rr(lambda(3:4))];
        end
        function phiLambda_rp = phiLambda_rp(cons,lambda)
            % partial derivative of (phi_r*lambda) with respect to p (orientation)
            % following technique from get.phi_r and get.phi_p
            % phiLambda_rp : [24x8] normally, unless grounded, then [12x4]
            % inputs:
            %    lambda : [4x1] vector of lambda values, corresponding to
            %    the subconstraints
            
            phiLambda_rp = [cons.subCons{1}.phiLambda_rp(lambda(1:2)); cons.subCons{2}.phiLambda_rp(lambda(3:4))];
        end    
        function phiLambda_pr = phiLambda_pr(cons,lambda)
            % partial derivative of (phi_p*lambda) with respect to r (position)
            % following technique from get.phi_r and get.phi_p
            % phiLambda_pr : [32x6] normally, unless grounded, then [16x3]
            % inputs:
            %    lambda : [4x1] vector of lambda values, corresponding to
            %    the subconstraints
            
            phiLambda_pr = [cons.subCons{1}.phiLambda_pr(lambda(1:2)); cons.subCons{2}.phiLambda_pr(lambda(3:4))];
        end
        function phiLambda_pp = phiLambda_pp(cons,lambda)
            % partial derivative of (phi_p*lambda) with respect to p (orientation)
            % following technique from get.phi_r and get.phi_p
            % phiLambda_pp : [32x8] normally, unless grounded, then [16x4]
            % inputs:
            %    lambda : [4x1] vector of lambda values, corresponding to
            %    the subconstraints
            
            phiLambda_pp = [cons.subCons{1}.phiLambda_pp(lambda(1:2)); cons.subCons{2}.phiLambda_pp(lambda(3:4))];
        end 
    end %methods
end %class